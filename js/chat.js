...
if (!location.hash) {
  ...
  function startWebRTC(isOfferer) {
    ...
    // Quando recebemos a notificação de fluxo de vídeo recebido, apresentamos ele dentro do elemento #videoRemoto
    pc.ontrack = event => {
      const stream = event.streams[0];
      if (!videoRemoto.srcObject || videoRemoto.srcObject.id !== stream.id) {
    ...
    navigator.mediaDevices.getUserMedia({
      ...
      // Mostra o fluxo de vídeo aberto no elemento #videoLocal
